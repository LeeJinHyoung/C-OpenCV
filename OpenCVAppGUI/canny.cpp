...
std::vector<int> rgbToGrayscale(std::vector<int>& pixels, int sizeRows, int sizeCols, int sizeDepth) {
    ...
    for (int i = 0; i < sizeRows; i++) {
        for (int j = 0; j < sizeCols; j++) {
            ...
            int r = pixels[i * sizeCols * sizeDepth + j * sizeDepth + 0];
            int g = pixels[i * sizeCols * sizeDepth + j * sizeDepth + 1];
            int b = pixels[i * sizeCols * sizeDepth + j * sizeDepth + 2];
            float y = r * 0.299 + g * 0.587 + b * 0.114;
            pixelsGray[i * sizeCols + j] = (int)(y);
        }
    }
    return pixelsGray;
}
...
std::vector<int> cannyFilter(std::vector<int>& pixels, int sizeRows, int sizeCols, int sizeDepth, double lowerThreshold, double higherThreshold) {
    ...
    cv::Mat in(sizeRows, sizeCols, CV_32SC1, &pixels[0]);
    ...
    for (int i = 1; i < sizeRows - 1; i++) {
        for (int j = 1; j < sizeCols - 1; j++) {
            ...
            for (int x = -1; x <= 1; x++) {
                for (int y = -1; y <= 1; y++) {
                    gxValue += (gx[1 - x][1 - y] * (double)(pixels[(i + x) * sizeCols + j + y]));
                    gyValue += (gy[1 - x][1 - y] * (double)(pixels[(i + x) * sizeCols + j + y]));
                }
            }

            // calculate G and theta
            G[i * sizeCols + j] = std::sqrt(std::pow(gxValue, 2) + std::pow(gyValue, 2));
            double atanResult = atan2(gyValue, gxValue) * 180.0 / CV_PI;// 3.14159265;
    ...
    cv::Mat gX(sizeRows, sizeCols, CV_64FC1, &G[0]);
    gX.convertTo(gX, CV_8UC1);

    // non-maximum suppression
    for (int i = 1; i < sizeRows - 1; i++) {
        for (int j = 1; j < sizeCols - 1; j++) {
            if (theta[i * sizeCols + j] == 0 || theta[i * sizeCols + j] == 180) {
                if (G[i * sizeCols + j] < G[i * sizeCols + j - 1] || //left
                    G[i * sizeCols + j] < G[i * sizeCols + j + 1]) { //right
                    G[i * sizeCols + j] = 0;
                }
            }
            else if (theta[i * sizeCols + j] == 45 || theta[i * sizeCols + j] == 225) {
                if (G[i * sizeCols + j] < G[(i + 1) * sizeCols + j - 1] || //left - bottom
                    G[i * sizeCols + j] < G[(i - 1) * sizeCols + j + 1]) { //right - top
                    G[i * sizeCols + j] = 0;
                }
            }
            ...
            //normalization (scaled ... now value -> new value)
            pixelsCanny[i * sizeCols + j] = (int)(G[i * sizeCols + j] * (255.0 / largestG));
        }
    }

    cv::Mat gNMS(sizeRows, sizeCols, CV_32SC1, &pixelsCanny[0]);
    gNMS.convertTo(gNMS, CV_8UC1);
    ...
    //std::vector<int> pixelsCanny(sizeRows* sizeCols);
    cv::Mat edge(sizeRows, sizeCols, CV_32SC1, &pixelsCanny[0]);
